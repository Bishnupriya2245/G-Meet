var AppProcess = (function () {
        ...
        for (var con_id in peers_connection_ids) {
            if (connection_status(peers_connection[con_id])) {
                if (rtp_senders[con_id] && rtp_senders[con_id].track) {
                    ...
                    rtp_senders[con_id] = await peers_connection[con_id].addTrack(track);
                }
    ...
    async function videoProcess(newVideoState) {
        ...
        console.log("22233", removeVideoStream(rtp_vid_senders));
        ...
        socket.on("inform_me_about_other_users", function (other_users) {
            if (other_users) {
                for (var i = 0; i < other_users.length; i++) {
                    if ($("#" + other_users[i].connectionId).length === 0) {
